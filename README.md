...
`x86_64-unknown-linux-gnu`, and `x86_64-pc-windows-msvc` — each built,
smoke-tested, and packaged on its own native GitHub-hosted runner (see
`.github/workflows/release.yml`), not cross-compiled. **On Windows,
`kupl native`/`kupl bundle` don't work yet** (see Known Limitations in
`docs/PRODUCTION.md`) — everything else (`kupl run`, `kupl run --vm`,
`kupl check`, `kupl build`, `kupl fmt`, `kupl test`) is unaffected.
Intel macOS isn't published yet — build from source with Option A/B
above; the toolchain is a single dependency-free Rust crate.
