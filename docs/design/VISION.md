...
- `ASYNC.md` — design sketch and implementation log for real concurrency between component instances; its core plan shipped as `concurrent component` (and the `agent` keyword built on top of it — see `AGENTS.md`). The FULL vision (implicit M:N scheduling for every ordinary component) remains open — closing that residual gap between §4's "M:N work-stealing scheduler" claim and the still-mostly-synchronous default runtime.
- `AGENTS.md` — `agent`: a higher-level actor purpose-built to represent a human-like co-worker, with commitments (`protocol`/`follows`), a choice of concurrency backing (`weight`), memory that outlives a single run (`durable`), and a judgment boundary (`deterministic`). Implemented; normative spec `docs/reference/LANGUAGE-REFERENCE.md` §7.3.
