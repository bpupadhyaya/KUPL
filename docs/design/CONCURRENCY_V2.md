...
| **Distribution** | none (external only) | none (external only) | **built into the language/VM** | none (weak ecosystem) | none shipped (aspirational only) | yes (`distributed actor`, 2022, niche) | none | none | **shipped, 2026-09-01 — `weight distributed`/`kupl node`, single-node manual addressing, authenticated + ChaCha20-Poly1305 encrypted from `Spawn` onward as of 2026-09-04 (see `DISTRIBUTION.md`); not the full addressing/clustering Erlang/OTP has, nor PKI-based mTLS** |
