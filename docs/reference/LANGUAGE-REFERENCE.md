...
| Kind | Examples | Notes |
|---|---|---|
| Int | `42`, `1_000_000`, `0xFF`, `0b1010`, `-7` | 64-bit signed; hex/binary + `_` separators; overflow **panics** (never wraps). Bitwise ops via methods (`.band`/`.bor`/…); format back with `.to_hex`/`.to_binary`/`.to_radix` |
