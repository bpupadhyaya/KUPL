...
**Version:** 0.3.0 · **Status:** normative for the implemented language
...
- `await expr` is accepted and currently evaluates `expr` directly (expose
  calls are synchronous in v0.3.0; true asynchrony is **[design]**).
...
- Effect names are hierarchical: declaring `db` covers `db.read`; declaring
  ...
  v0.3.0: `print` / `eprint` (`io`); the file builtins `read_file` /
...
- Recursion (incl. mutual) is fully supported. Functions are first-class:
  ...
  (their effects are not tracked in v0.3.0 — documented limitation).
...
- Tool effects are **not** statically propagated to the ai fun in
  v0.3.0 (a documented limitation, like calls through variables).
...
is not statically required to declare `ai` — the same v0.3.0 limitation
