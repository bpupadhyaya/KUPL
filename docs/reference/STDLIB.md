...
**Version:** 0.1.0. Everything here is built into the language runtime and
...
| `cap_net_root()` | `() -> CapNet` | the unrestricted root network capability; only callable directly in `fun main`'s own top-level body (`K0304` otherwise) — see Capabilities below |
