...
model. (The full numeric surface — including sized integers and `f32` — now compiles;
only `ai fun` and the JSON/CSV/HTTP builtins still defer with a clear message,
use `bundle`. KValue unboxing for tighter numeric loops is a future performance
arc.)
