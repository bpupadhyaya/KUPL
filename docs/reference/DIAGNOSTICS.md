...
**Version:** 0.2.0. Every diagnostic has a stable code, a precise source
...
| K0296 | E | a call to a private `fun` that itself performs a blocking builtin call as its own top-level statement is subject to the SAME restriction as K0295 — may only appear as the entire right-hand side of a top-level `let` — see `docs/design/ASYNC_IO.md` |
...
| K1008 | E | `deterministic` is only valid on an `agent`, not a plain `component`/`concurrent component` |
| K1009 | E | a `deterministic` agent's own exposed fun performs the `ai` effect |
| K1010 | E | an agent's `follows` list names two DIFFERENT protocols that both declare a guard with the same name — resolves `docs/design/AGENTS.md` §3's own "multi-protocol composition" question for this one shape by making the collision a compile-time error (rename one of the guards) rather than silently letting whichever protocol is encountered last win |
