    ...
    fn call(&mut self, callee: &Expr, args: &[Arg], span: Span) -> Reg {
        if let ExprKind::Ident(name) = &callee.kind {
            ...
            let builtin = match (name.as_str(), args.len()) {
                ...
                ("text_embed", 2) => Some(BUILTIN_TEXT_EMBED),
                ("cosine_similarity", 2) => Some(BUILTIN_COSINE_SIMILARITY),
                ...
                ("log_debug", 1) => Some(BUILTIN_LOG_DEBUG),
                ("log_info", 1) => Some(BUILTIN_LOG_INFO),
                ("log_warn", 1) => Some(BUILTIN_LOG_WARN),
                ("log_error", 1) => Some(BUILTIN_LOG_ERROR),
